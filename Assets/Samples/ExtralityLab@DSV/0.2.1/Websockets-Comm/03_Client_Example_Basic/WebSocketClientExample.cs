...
public class WebSocketClientExample : MonoBehaviour
{
    ...
    public string serverIP = "10.204.0.101"; // Replace with your server's IP address
    public int serverPort = 8081; // Replace with your server's port number (8081 is the default)
    public FlipperXR flipper;// Reference to the puzzle piece in the scene
    [Range(0, 255)]
    ...
    public void IncomingMessageParser(String msg)
    {
        ...
        if(msg.Contains("button")) {
            if(valueParsed == "1") 
            {
                ...
                flipper.esp32Controlled = true; // This is an example of how you can use the incoming message to control a puzzle piece in the scene. In this case, we set the puzzle piece to be controlled by the ESP32 when the button is pressed.
                flipper.playOneTimeActivateAudio = true; // This is an example of how you can use the incoming message to control a puzzle piece in the scene. In this case, we play the one-time activation audio when the button is pressed.
            ...
            if(valueParsed == "0") 
            {
                ...
                flipper.esp32Controlled = false; // This is an example of how you can use the incoming message to control a puzzle piece in the scene. In this case, we set the puzzle piece to not be controlled by the ESP32 when the button is released.
                flipper.playOneTimeDeactivateAudio = true; // This is an example of how you can use the incoming message to control a puzzle piece in the scene. In this case, we play the one-time deactivation audio when the button is released.
